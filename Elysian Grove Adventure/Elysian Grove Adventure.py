...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        self.damage_reduction_end_times = [end_time for end_time in self.damage_reduction_end_times if
...
class Ripple(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for bossenemy in bossenemies:
            distance = ((self.rect.centerx - bossenemy.rect.centerx) ** 2 + (
                        self.rect.centery - bossenemy.rect.centery) ** 2) ** 0.5
            if distance < nearest_distance:
                nearest_distance = distance
                nearest_enemy = bossenemy
        for malakar in malakar_group:
            distance = ((self.rect.centerx - malakar.rect.centerx) ** 2 + (
                        self.rect.centery - malakar.rect.centery) ** 2) ** 0.5
            if distance < nearest_distance:
                nearest_distance = distance
...
def draw_inventory(surface, player):
    ...
    draw_text(surface, f'Collected Fruits:', 18, WIDTH // 2, PLAYABLE_HEIGHT + 10, WHITE)
    ...
    if player.damage == 100:
        draw_text(surface, 'Status: Increased Damage', 18, x_offset + 30, PLAYABLE_HEIGHT + 30, GREEN)
    ...
    with open("savegame.dat", "wb") as f:
        pickle.dump({
            'player': {
                ...
                'speed_boost_end_time': player.speed_boost_end_time,
                'damage_reduction': player.damage_reduction,
                'damage_reduction_end_times': player.damage_reduction_end_times,
            ...
            'bossenemies': [{
                'type': type(bossenemy).__name__,
                'health': bossenemy.health,
                'position': (bossenemy.rect.x, bossenemy.rect.y)
            } for bossenemy in bossenemies],
            'malakar_group': [{
                'type': type(malakar).__name__,
                'health': malakar.health,
                'position': (malakar.rect.x, malakar.rect.y)
            } for malakar in malakar_group]
        }, f)
...
def load_game():
    ...
    if os.path.exists("savegame.dat"):
        with open("savegame.dat", "rb") as f:
            ...
            player.invulnerable = player_data['invulnerable']
            player.last_hit = player_data['last_hit']
            player.invuln_end_time = player_data['invuln_end_time']
            player.melon_end_time = player_data['melon_end_time']
            player.last_regen_time = player_data['last_regen_time']
            player.special_attack_ready = player_data['special_attack_ready']
            player.special_attack_time = player_data['special_attack_time']
            player.flamefruit_end_time = player_data['flamefruit_end_time']
            player.flamefruit_active = player_data['flamefruit_active']
            player.flamefruit_position = player_data['flamefruit_position']
            player.speed_boost_end_time = player_data['speed_boost_end_time']
            player.damage_reduction = player_data['damage_reduction']
            player.damage_reduction_end_times = player_data['damage_reduction_end_times']
            player.rect.topleft = player_data['position']
            ...
            for enemy_data in data['enemies']:
                ...
                if enemy_type == 'NightCrawler':
                    enemy = NightCrawler(*enemy_data['position'])
                enemy.health = enemy_data['health']
                enemies.add(enemy)
                all_sprites.add(enemy)

            for bossenemy_data in data['bossenemies']:
                bossenemy_type = bossenemy_data['type']
                if bossenemy_type == 'BossEnemy':
                    bossenemy = BossEnemy(*bossenemy_data['position'])
                bossenemy.health = bossenemy_data['health']
                bossenemies.add(bossenemy)
                all_sprites.add(bossenemy)

            for malakar_data in data['malakar_group']:
                malakar_type = malakar_data['type']
                if malakar_type == 'Malakar':
                    malakar = Malakar(*malakar_data['position'])
                malakar.health = malakar_data['health']
...
while running:
    ...
    if paused:
        ...
        draw_inventory(screen, player)
        draw_text(screen, f'Speed: {player.speed}', 18, WIDTH // 2, HEIGHT // 2 + 60, WHITE)
        draw_text(screen, f'Damage: {player.damage}', 18, WIDTH // 2, HEIGHT // 2 + 80, WHITE)
        draw_text(screen, f'Damage Reduction: {player.damage_reduction}', 18, WIDTH // 2, HEIGHT // 2 + 100, WHITE)
        draw_text(screen, f'Level: {player.level}', 18, WIDTH // 2, HEIGHT // 2 + 120, WHITE)
        draw_text(screen, f'Experience: {player.experience}', 18, WIDTH // 2, HEIGHT // 2 + 140, WHITE)
        draw_text(screen, f'Health: {int(player.health)}', 18, WIDTH // 2, HEIGHT // 2 + 160,
                  WHITE)  # Display health as an integer
